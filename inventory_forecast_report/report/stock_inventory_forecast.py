...
class StockInventoryForecast(models.Model):
    ...
    cumulative_quantity = fields.Float('Total')
        ...
        self._cr.execute("""
            CREATE VIEW stock_inventory_forecast AS (
              SELECT MIN(id) AS id,
                   ...
                   sum(sum(quantity)) OVER (PARTITION BY product_id ORDER BY date) AS cumulative_quantity,
                       ...
                       (SELECT MIN(id) AS id,
                        ...
                        FROM (
                                (SELECT sale_order_line.id AS id,
                                        ...
                                        concat(sorder.name, sale_order_line.id) as SOURCE
                              ...
                              UNION ALL
                                (SELECT (-1) * sale_order_line.id AS id,
                                        sorder.company_id AS company_id,
                                        sale_order_line.product_id AS product_id,
                                     ...
                                     JOIN sale_order_line ON sale_order_line.order_id = sorder.id
                                     JOIN product_product ON product_product.id = sale_order_line.product_id
                                     WHERE sorder.state = 'sale' and (sale_order_line.product_uom_qty - sale_order_line.qty_delivered) != 0
                                )
                                    ) AS SaleOrderTable GROUP BY company_id, product_id, SOURCE, date
                              UNION ALL
                                (SELECT purchase_order_line.id AS id,
                                        ...
                                        concat(porder.name, purchase_order_line.id) as SOURCE
                                    ...
                                    WHERE porder.state not in ('cancel') and (purchase_order_line.product_qty - purchase_order_line.qty_received) != 0
                                )
                                    )AS PSOTable GROUP BY company_id, product_id, SOURCE, date
                       )
                        UNION ALL
                        (SELECT (-1) * MIN(id) AS id,
                                company_id,
                                product_id,
                                SUM(quantity) AS quantity,
                                date,
                                 ...
                                 (SELECT stock_move.id AS id,
                                  ...
                                  WHERE quant.qty>0
                                    ...
                                    AND (NOT (source_location.company_id IS NULL
                                         ...
                                         OR source_location.usage NOT IN ('internal',
                                                                          'transit')) )
                               UNION ALL
                                 (SELECT
                                ...
                                GROUP BY company_id, product_id, SOURCE, date
                    )""")
